...
function renderBannerPanel() {
  ...
  if (banner) {
    wrap.innerHTML = `
      <img src="${banner.image}" class="preview-img" alt="Banner">
    `;
...
function openProductModal(mode, product) {
  ...
  if (pendingProductImage) {
    wrap.innerHTML = `<img src="${pendingProductImage}" class="preview-img" alt="Preview">`;
  } else {
    wrap.innerHTML = '';
  }
  ...
  tbody.innerHTML = orders.map(o => `
    <tr>
      ...
      <td>
        ...
        <div style="font-size:11.5px;color:var(--text-3)">${escapeHtml(o.customer?.email || '')}</div>
      </td>
      ...
      <td>
        <div class="table-actions">
          ...
          ${o.status === 'pending' ? `
            <button class="btn btn-primary btn-sm" onclick="markPaid('${o.id}')">Paid</button>
          ` : ''}
        </div>
      </td>
    </tr>
